...
class Pelican extends Server
{
    ...
    public function getProductConfig($values = []): array
    {
        ...
        foreach ($eggs['data'] as $egg) {
            $allEggList[$egg['attributes']['id']] = $egg['attributes']['name'];
            foreach ($egg['attributes']['tags'] ?? [] as $tag) {
                $eggTagSet[$tag] = $tag;
            }
        }

        $eggList = [];
        if (isset($values['egg_tag']) && $values['egg_tag'] !== '') {
            foreach ($eggs['data'] as $egg) {
                if (in_array($values['egg_tag'], $egg['attributes']['tags'] ?? [])) {
        ...
        return [
            [
                ...
                'label' => 'Tag(s)',
                'type' => 'select',
                'description' => 'Tag(s) to filter nodes for deployment',
                'options' => $tagSet,
                'multiple' => true,
                'database_type' => 'array',
                'required' => false,
            ...
            [
                ...
                'type' => 'select',
                'options' => $eggTagSet,
                'description' => 'Egg Tag to filter eggs by',
                'required' => false,
                'live' => true,
            ],
            [
    ...
    public function createServer(Service $service, $settings, $properties)
    {
        ...
        if (isset($settings['location'])) {
            $settings['tags'] = [$settings['location']];
        }
